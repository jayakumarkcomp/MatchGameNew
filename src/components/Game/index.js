...
class Game extends Component {
  state = {
    activeTab: 'FRUIT',
    dispImage: imagesList[0],
    toDisplayImageList: imagesList.slice(1, imagesList.length),
  }

  getRandomImage = () => {
    this.setState(prevState => {
      const {toDisplayImageList} = prevState
      if (toDisplayImageList.length === 0) {
        const {onGameOver} = this.props
        onGameOver()
        return {dispImage: prevState.dispImage}
      }
      const randomPick = Math.floor(Math.random() * toDisplayImageList.length)
      const imageItem = toDisplayImageList[randomPick]
      toDisplayImageList.slice(randomPick, randomPick + 1)
      return {
        dispImage: imageItem,
        toDisplayImageList: prevState.toDisplayImageList.filter(
  ...
  render() {
    const {activeTab, dispImage} = this.state
    const filteredTabsList = imagesList.filter(
      item => item.category === activeTab,
    )
    return (
      <div className="game-container1">
        <div className="display-image-container1">
          <img
            className="display-image1"
            src={dispImage.imageUrl}
            alt="match"
          />
        </div>
        <ul className="tabs-container1">
          {tabsList.map(item => (
            <li className="tab-item1" key={item.tabId}>
              <button
                className={`tab-button1 ${
                  item.tabId === activeTab ? 'active-tab-button1' : ''
                }`}
                type="button"
                onClick={() => this.onClickingTab(item.tabId)} // sending props without the event handler
              >
                {item.displayText}
              </button>
            </li>
        ...
        <ul className="thumbnails-container1">
          {filteredTabsList.map(item => (
            <li className="thumbnail-item1" key={item.id}>
              <button
                className="thumbnail-button1"
                type="button"
                onClick={() =>
                  this.onClickingThumbnail(item.id === dispImage.id)
                }
              >
                <img
